...
function delAll(){
  ...
  const noOfDeletedRows = checkedRows.length;       
  checkedRows.forEach(function(checkbox) {
    // console.log(typeof checkbox.id);
    del(checkbox.id);
  });
  if(ind-rowsPerPage>=0){
    ind-=rowsPerPage;
  }
  if(ind+rowsPerPage<allData.length){
    ind+=rowsPerPage;
  }
  else{
    ind;
...
function pagination(userData){
  ...
  div.innerHTML = `
    <span>${Math.ceil((ind+1)/rowsPerPage)} Page of ${Math.ceil(userData.length/rowsPerPage)} Pages</span>
  ...
  if(Math.ceil((ind+1)/rowsPerPage) === Number(1)){
    document.querySelector('#prev').disabled = true;
  }

  //Display Next Page if on last page
  if(Math.ceil((ind+1)/rowsPerPage) === Math.ceil(userData.length/rowsPerPage)){
